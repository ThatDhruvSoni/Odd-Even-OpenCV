...
def ai_batting_loop(cap, ai, target=None):
    score = 0
    last_round_time = 0
    round_interval = 2.0
    ...
    while True:
        ...
        cv2.putText(frame, "You're bowling right now!", (frame.shape[1] - cv2.getTextSize("You're bowling right now!", cv2.FONT_HERSHEY_SIMPLEX, 1.5, 2)[0][0] - 20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (100, 255, 100), 2)
        ...
        if player_move is not None and time_since_last_round >= round_interval:
            ...
            last_result, points = play_round(ai_guess, player_move)
        ...
        cv2.putText(frame, f"AI Score: {score}", (10, frame.shape[0] - 20),
